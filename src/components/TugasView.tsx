            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-6">
              ...
              {selectedSubmissionForGrading && (
                <div 
                  ...
                  onClick={(e) => {
                    if (e.target === e.currentTarget) setSelectedSubmissionForGrading(null);
                  }}
                >
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 max-w-lg w-full shadow-2xl flex flex-col max-h-[90vh] my-auto overflow-hidden animate-in fade-in zoom-in-95 duration-150">
                    {/* Header Modal */}
                    <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/70 dark:bg-slate-800/50 shrink-0">
                      <div>
                        <h4 className="font-display font-bold text-slate-800 dark:text-white text-sm">Penilaian & Koreksi Tugas</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                          Siswa: <strong className="text-indigo-600 dark:text-indigo-400 font-semibold">{selectedSubmissionForGrading.siswaNama}</strong>
                        </p>
                      </div>
                      <button
                        type="button"
                        onClick={() => setSelectedSubmissionForGrading(null)}
                        className="p-1.5 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-200/60 dark:hover:bg-slate-800 dark:hover:text-slate-200 transition"
                        title="Tutup Modal"
                      >
                        <X className="h-5 w-5" />
                      </button>
                    ...
                    <div className="p-5 space-y-4 overflow-y-auto flex-1">
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">
                          Perolehan Nilai (Maksimal {selectedTugas.totalPoin})
                        </label>
                        <input
                          type="number"
                          required
                          ...
                          className="block w-full border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 text-sm font-bold text-slate-800 dark:text-slate-100 focus:outline-indigo-500 bg-white dark:bg-slate-800"
                        />
                      </div>
                      ...
                      <div className="space-y-1.5 text-left">
                        ...
                        <WysiwygEditor
                          ...
                          heightClass="min-h-[100px] max-h-[160px]"
                        />
                      </div>
                    </div>

                    {/* Fixed Footer Buttons */}
                    <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/50 flex justify-end items-center gap-2 shrink-0">
                      <button
                        type="button"
                        onClick={() => setSelectedSubmissionForGrading(null)}
                        className="border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold px-4 py-2 rounded-xl text-xs transition"
                      >
                        Batal
                      </button>
                      <button
