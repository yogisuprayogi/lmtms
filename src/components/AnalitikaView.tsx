              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="50%" cy="50%" outerRadius="70%" data={analitika.pencapaianElemen}>
                    ...
                    <Radar name="Skor Rerata" dataKey="nilai" stroke="#0d9488" fill="#0d9488" fillOpacity={0.35} />
                    <Tooltip contentStyle={{ fontSize: "11px", borderRadius: "12px" }} />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
          ...
          <div className="bg-white p-5 border border-slate-200 rounded-2xl shadow-xs" id="elemen-pembelajaran-card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" id="elemen-cards-grid">
              {ELEMEN_INFORMATIKA.map((el) => {
                ...
                return (
                  <div key={el.kode} className="p-4 border border-slate-200 bg-slate-50/50 hover:bg-white hover:border-slate-300 rounded-xl transition shadow-2xs flex flex-col justify-between group" id={`elemen-card-${el.kode}`}>
                    <div>
                      ...
                      <h4 className="font-display font-bold text-xs text-slate-800 mt-3 group-hover:text-indigo-600 transition">{el.nama}</h4>
                      <p className="text-[10px] text-slate-500 mt-1 line-clamp-3 leading-relaxed">{el.deskripsi}</p>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col justify-between">
              <div>
                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5 text-rose-600">
                  <AlertTriangle className="h-4.5 w-4.5" />
                  <span>Siswa Perlu Pembimbingan Remedial</span>
                </h4>
                <div className="space-y-2 max-h-[220px] overflow-y-auto">
                  {remediationList.length > 0 ? (
                    remediationList.map((s) => (
                      <div key={s.id} className="p-2.5 bg-rose-50/50 border border-rose-100/60 rounded-xl flex items-center justify-between text-xs">
                        <div>
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-[9px] text-slate-400 block">KKTP &lt; 75</span>
                        </div>
                      </div>
                    ))
                  ) : (
              ...
              <div className="text-[10px] text-slate-400 mt-4 leading-normal">
                Segera jadwalkan program remedial luring atau berikan alternatif tugas mandiri via menu <b>AI Pengajar &gt; AI Soal</b>.
              </div>
            </div>
